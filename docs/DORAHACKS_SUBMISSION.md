...
## One-liner
A pre-signing policy firewall for autonomous Casper agents that blocks unsafe wallet actions and anchors decision proofs on Casper Testnet.
...
## Full Description
Autonomous AI agents are becoming capable of paying APIs, calling contracts, rebalancing portfolios, managing treasuries, and interacting with DeFi/RWA workflows. Once an agent has wallet access, a prompt injection or faulty tool call can trigger irreversible on-chain damage.
...
For auditability, each decision produces deterministic hashes: `intent_hash`, `policy_hash`, `evidence_hash`, and `decision_hash`. The MVP includes a Rust/WASM Casper session anchor that writes compact proof fields to Casper Testnet named keys without exposing private prompt details.
...
- Casper Testnet: includes a live transaction-producing on-chain anchor.
- Open source: public GitHub repository with README and usage instructions.
- Long-term impact: reusable safety layer for Casper's agent economy.

## Requirement Checklist

- Working prototype: Yes. Dashboard, CLI demo, policy engine, tests, and build scripts are included.
- Casper Testnet on-chain component: Yes. The BLOCK decision was anchored on Casper Testnet.
- Transaction-producing behavior: Yes. The deploy writes AgentShield decision data into Casper account named keys.
- Open-source repository: Yes. The GitHub repository includes source code, README, usage instructions, tests, CI, and deployment documentation.
- Public demo video: Add a public YouTube or Loom URL before final submission. Repo MP4 evidence is included at `demo/casper-agentshield-demo.mp4`.
...
- Working Smart Contracts: live Casper Testnet session deploy plus Odra proof-registry source.
